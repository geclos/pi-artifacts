...
import { artifactDir, walkFiles } from "../store/artifactStore.js";
...
export async function validateArtifact(cwd: string, id: string, strict = false): Promise<ValidationResult> {
  ...
  if (!existsSync(index)) {
    issues.push({ severity: "error", code: "missing_entry", message: "Missing dist/index.html." });
  ...
  for (const [re, code, message] of checks) if (re.test(html)) issues.push({ severity: strict ? "error" : "warning", code, message, file: "index.html" });
  if (!html.includes("Pi Artifact UI") && !html.includes("--ds-background-100") && !html.includes("pi-shell")) {
    issues.push({ severity: strict ? "error" : "warning", code: "missing_design_system", message: "Artifact does not appear to include the Pi Artifact UI theme/classes.", file: "index.html" });
  }
  const rootOverrides = (html.match(/--ds-[a-z0-9-]+\s*:/gi) || []).length;
  if (rootOverrides > 140) issues.push({ severity: "warning", code: "excessive_token_overrides", message: `Found ${rootOverrides} design-token declarations; avoid excessive custom token overrides.`, file: "index.html" });
